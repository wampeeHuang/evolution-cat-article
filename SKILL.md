...
⏸ 暂停等人审：Step 5c 后（事实复验通过，配图前最后确认）。配图 API 有成本。

> **改后重检规则**：改完 draft-v3.md 任何内容后，按改动范围决定重跑哪些门禁。小改（修措辞）→ 3.5+4；中改（换例子/调数据/改lede）→ 3.5+4+5a+5c；大改（动结构）→ 从 Step 2 全跑。详见 production-sop.md §改后重检规则。
